...
def step14_subscribe_waba() -> bool:
    ...
    if not WABA_ID:
        print("\n[FAIL] WHATSAPP_WABA_ID is not set in .env")
    ...
    if result.get("success"):
        print("\n[OK] Step 14 done -- WABA subscribed to app webhooks via user token")
        return True
    ...
    if APP_ID and APP_SECRET:
        ...
        if result.get("success"):
            print("\n[OK] Step 14 done -- WABA subscribed to app webhooks via app token")
            return True
    else:
        print("\n  (skipping app-token attempt -- WHATSAPP_APP_ID not set in .env)")

    print("\n[FAIL] Step 14 failed. Check the error above.")
...
def step15_send_message(recipient: str) -> bool:
    ...
    if not PHONE_NUMBER_ID:
        print("[FAIL] WHATSAPP_PHONE_NUMBER_ID not set in .env")
        return False
    if not TOKEN:
        print("[FAIL] WHATSAPP_TOKEN not set in .env")
        return False
    ...
    if result.get("messages"):
        ...
        print(f"\n[OK] Step 15 done -- message sent, id={msg_id}")
    ...
    code = err.get("code")
    if code == 131047:
        print("\n[FAIL] Error 131047: 24-hour messaging window is closed.")
        print("   Send a message TO the test number from your phone first,")
        print("   then re-run this script.")
    elif code == 190:
        print("\n[FAIL] Error 190: access token is invalid or expired.")
        print("   Regenerate WHATSAPP_TOKEN (Step 8) or exchange for 60-day (Step 10).")
    else:
        print("\n[FAIL] Step 15 failed -- see error above.")
    return False
