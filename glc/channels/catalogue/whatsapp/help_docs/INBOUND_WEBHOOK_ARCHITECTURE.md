...
**Script location:** `glc/channels/catalogue/whatsapp/demo_webhook_server.py`
— shipped alongside `adapter.py` (not under `help_docs/`) since it's the
only thing that makes the adapter reachable by a real webhook today;
`help_docs/` stays reference-only material that doesn't ship in the
final PR.
...
| `glc/channels/catalogue/whatsapp/demo_webhook_server.py` | Approach 2 server — shipped, not under `help_docs/` |
